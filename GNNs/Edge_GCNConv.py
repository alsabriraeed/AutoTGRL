...
from GNNs.message_passing import MessagePassing       
...
class Edge_GCNConv(MessagePassing):
    def __init__(self, in_channels, out_channels, aggr='add', edge_dim=1,concat=True, bias=True):      
        ...
        self.edge_update = torch.nn.Parameter(torch.Tensor(out_channels , out_channels))
        self.concat = concat
    ...
    def forward(self, x, edge_index, edge_attr):
        ...
        # 1. Linearly transform node feature matrix  (XÃƒÅ½Ã‹Å“)
        x = torch.matmul(x, self.weight)  # N x emb(out) = N x emb(in) @ emb(in) x emb(out)
        # print('x1', x.shape)
        # print('edge_attr1', edge_attr.shape)            
        ...
        # print('edge_attr2', edge_attr.shape)    
    ...
    def message(self, x_i, x_j, size_i, edge_attr, norm):   # 4.1 Normalize node features (concat edge_attr)
        ...
        # print('x_j3', x_j.shape)                
        # print('edge_attr3', edge_attr3.shape)       
