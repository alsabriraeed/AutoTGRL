...
class Trainer(object):
    ...
    def build_model(self):
        ...
        if self.args.search_mode == "micro":           
            ...
            from TGRL_SearchSpace.search_space import IncrementSearchSpace, GraphclassSearchSpace
            if self.args.model_type =='transductive':
                       
                search_space_cls = IncrementSearchSpace()                  
            else:     
                # search_space_cls = GraphclassSearchSpace()      
                search_space_cls = IncrementSearchSpace()               
            search_space = search_space_cls.get_search_space()        
            from search_algrithm.AutoTGRL_controller import SimpleNASController
            if self.args.model_type =='transductive':       
                from classification.Textclassification import Text_Classification
            else:
                from classification.Inductive_Textclassification import Text_Classification
            self.submodel_manager = Text_Classification(self.args)      
            ...
            if hasattr(self.args, "predict_hyper") and self.args.predict_hyper:
                self.action_list = action_list + ["learning_rate", "dropout", "weight_decay", "hidden_unit","Bert_style_model","bert_lr"]      
    ...
    def form_gnn_info(self, gnn):       
        if self.args.search_mode == "micro":
            ...
            if self.args.predict_hyper:
                actual_action["action"] = gnn[:-6]     
                actual_action["hyper_param"] = gnn[-6:]
            else:
    ...
    def train_controller(self):   
        ...
        for step in range(self.args.controller_max_step):
            ...
            # adv = utils.get_variable(adv, requires_grad=False)
...
# torch.cuda.empty_cache()
    ...
    def derive_from_history(self):
        ...
        for line in lines:
            ...
            results.append((actions, float(val_score)))
